...
def prepare_data(dataset_names=get_dataset_names()):
    ...
    for dataset in DATASETS:
        if not dataset.get_dataset_name() in dataset_names:
            continue
        print(f"--- Processing dataset: {dataset.get_dataset_name()} ---")
        data_frame = dataset.load_raw_dataset()
        d = preprocess(dataset, data_frame)

        for k, v in d.items():
            _write_to_file(dataset.get_filename(k), v)


def _write_to_file(filename, dataframe):
...
def preprocess(dataset, data_frame):
    ...
    if missing_data_count > 0:
        print(f"Missing Data: {str(missing_data_count)} rows removed from dataset "
    ...
    if len(sensitive_attrs) > 1:
        ...
        processed_data = processed_data.assign(
            temp_name=processed_data[sensitive_attrs].apply('-'.join, axis=1))
    ...
    processed_binsensitive = _make_sensitive_attrs_binary(
    ...
    processed_categorical_binsensitive = _make_sensitive_attrs_binary(
    ...
    processed_binsensitive = _make_class_attr_num(processed_binsensitive, class_attr, pos_val)
...
def _make_sensitive_attrs_binary(dataframe, sensitive_attrs, privileged_vals):
...
def _make_class_attr_num(dataframe, class_attr, positive_val):
